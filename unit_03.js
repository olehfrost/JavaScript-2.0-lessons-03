...
function f3(){
   let a1 = +document.querySelector('.i-31').value;
   let a2 = +document.querySelector('.i-32').value;
...
function f8(){
   ...
   switch (data) {
      case 1: out.textContent = 'one';
         break;
      case 2: out.textContent = 'two';
         break;
      case 3: out.textContent = 'three';
         break;
   }
}
...
function f9(){
   ...
   if (data >= 1 && data <= 32) {
      out.innerHTML = '1';
   } else if (data >= 33 && data <= 43) {
      out.innerHTML = '2';
   } else if (data >= 44 && data <= 64) {
      out.innerHTML = '3';
   }
   else {
      out.innerHTML = '0';
...
function f13(){
   let data = document.querySelector('.i-130').value;
   let out = document.querySelector('.out-13');
   out.textContent = typeof (data);
}
...
// Дана кнопка.b - 15, при нажатии на которую срабатывает функция f15.Функция выводит в.out - 15 результат логических операций
// выбранных в 3-х select к числам выбранным в первом и втором select.
